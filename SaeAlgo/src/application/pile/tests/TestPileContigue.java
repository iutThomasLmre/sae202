...
class TestPileContigue {
    ...
    void testPileContigue() {
        assertTrue(new PileContigue<Double>().isVide());
    }
    ...
    void testEmpiler() {
        ...
        assertDoesNotThrow(()->pileEntier.empiler(Integer.valueOf(0)));
        
        PileContigue<Integer> grandePile = new PileContigue<>();
        for (int valeur = 1 ; valeur <= 100000 ; valeur++) {
            final Integer objet = Integer.valueOf(valeur);
            assertDoesNotThrow(()->grandePile.empiler(objet));
        }
    }
